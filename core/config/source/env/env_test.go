...
func TestEnv_Read(t *testing.T) {
	expected := map[string]map[string]string{
		"database": {
			"host":       "localhost",
			"password":   "password",
			"datasource": "db_user:password@tcp(localhost:port)/db?charset=utf8mb4&parseTime=True&loc=Local",
		},
	}

	os.Setenv("DATABASE_HOST", "localhost")
	os.Setenv("DATABASE_PASSWORD", "password")
	os.Setenv("DATABASE_DATASOURCE", "db_user:password@tcp(localhost:port)/db?charset=utf8mb4&parseTime=True&loc=Local")
...
func TestEnvvar_Prefixes(t *testing.T) {
	...
	os.Setenv("CORE_REGISTRY", "mdns")

	var prefixtests = []struct {
		prefixOpts   []source.Option
		expectedKeys []string
	}{
		{[]source.Option{WithPrefix("APP", "CORE")}, []string{"app", "core"}},
		{[]source.Option{WithPrefix("CORE"), WithStrippedPrefix("APP")}, []string{"database", "core"}},
		{[]source.Option{WithPrefix("CORE"), WithStrippedPrefix("APP")}, []string{"database", "core"}},
	}
...
func TestEnvvar_WatchNextNoOpsUntilStop(t *testing.T) {
	src := NewSource(WithStrippedPrefix("CORE_"))
	w, err := src.Watch()
	if err != nil {
		t.Error(err)
	}
