...
func LoginCheck(c *gin.Context) {
	...
	if isUser {
		cond := loginCheck == blog_const.USER_TYPE_ADMIN.Code || loginCheck == blog_const.USER_TYPE_DEV.Code
		if abort(c, cond, &r.FAIL_ADMIN) {
			return
		}
	} else if isAdmin {
		cond := loginCheck == blog_const.USER_TYPE_ADMIN.Code && int(user.ID) != blog_const.ADMIN_USER_ID
		if abort(c, cond, &r.FAIL_ADMIN) {
			return
		}
	} else {
...
func abort(c *gin.Context, cond bool, cm *r.CodeMsg) bool {
	if cond {
		...
		res.Code, res.Message = cm.CodeMsg()
